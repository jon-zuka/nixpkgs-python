...
let
  ...
  overrides = final: prev:
    let pkgs = final.callPackage ({ pkgs }: pkgs) { };
    in {
      ...
      cffi = prev.cffi.overrideAttrs (old: { buildInputs = [ pkgs.libffi ]; });

      numpy = prev.numpy.overrideAttrs (old: {
        nativeBuildInputs = old.nativeBuildInputs
          ++ [ pkgs.pkg-config pkgs.blas pkgs.lapack ];
      });

      # Use stub from nixpkgs
      cmake = final.callPackage ({ stdenv, python3Packages }:
        stdenv.mkDerivation {
          inherit (python3Packages.cmake)
            pname version src meta postUnpack setupHooks;

          nativeBuildInputs = [ final.pyprojectHook ]
            ++ final.resolveBuildSystem { flit-core = [ ]; };
        }) { };

      # Use stub from nixpkgs
      ninja = final.callPackage ({ stdenv, python3Packages }:
        stdenv.mkDerivation {
          inherit (python3Packages.ninja)
            pname version src meta postUnpack setupHook preBuild;
      ...
      maturin = final.callPackage ({ stdenv, pkgs, }:
        stdenv.mkDerivation {
          inherit (pkgs.maturin) pname version cargoDeps src meta;
      ...
      setuptools-scm = prev.setuptools-scm.overrideAttrs
        (old: { inherit (pkgs.python3Packages.setuptools-scm) setupHook; });
      ...
      # Use setup hook from nixpkgs (sets up build)
      meson-python = prev.meson-python.overrideAttrs
        (old: { inherit (pkgs.python3Packages.meson-python) setupHooks; });

      # Use setup hook from nixpkgs (pretends version)
      poetry-dynamic-versioning = prev.poetry-dynamic-versioning.overrideAttrs
        (old: {
      ...
      cython = prev.cython.overrideAttrs
        (old: { inherit (pkgs.python3Packages.cython) setupHook; });
      ...
      pdm-backend = prev.pdm-backend.overrideAttrs (old:
        {
          ...
          nativeBuildInputs = old.nativeBuildInputs
            ++ (final.resolveBuildSystem { importlib-metadata = [ ]; });
        });
  ...
  mkResolveBuildSystem = set:
    let
      resolveNonCyclic' =
        resolveNonCyclic (lib.attrNames workspace.deps.default) set;
    ...
    in spec:
    if spec != { } then
      map (name: set.${name}) (resolveNonCyclic' spec)
    else
      fallbackSystems;
